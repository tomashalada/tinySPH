...
def resample():
    for z in range(zdim):
        for x in range(xdim):
            ...
            deltar_resampled[xdim*z + x,0] = deltar[x*zdim + z,0]
            deltar_resampled[xdim*z + x,1] = deltar[x*zdim + z,1]
            deltar_resampled[xdim*z + x,2] = deltar[x*zdim + z,2]
...
def myGrid():
    with open(outputFileName, 'w') as f:
        with redirect_stdout(f):
            ...
            for i in range(xdim*ydim*zdim):
                print("", p_resampled[i])
            print("DELTA_R 3", xdim*ydim*zdim, " float")
            for i in range(xdim*ydim*zdim):
                print("", deltar_resampled[i,0], " ", deltar_resampled[i,1], " ", deltar_resampled[i,2])
