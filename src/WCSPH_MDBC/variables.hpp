...
template <typename T, typename S>
void BoundVars<T, S>::allocate(size_t n){
...
template <typename T, typename S>
void BoundVars<T, S>::initializeWithGeometryFile(std::string geoFileName){
  ...
  while (getline (geoFile, geoFileText)) {
    ...
    if(line == 0){
      this-> N = std::stoi(geoFileText);
      this-> BoundVars::allocate(this-> N); //is this ok?
      line++;
      continue;
    }
    ...
    this-> gn[line-1].x = std::stod(tokens[8]);
    this-> gn[line-1].y = std::stod(tokens[9]);
    this-> gn[line-1].z = std::stod(tokens[10]);

    line++;
    if(line == this-> N + 1){break;} //is this ok?
  }
